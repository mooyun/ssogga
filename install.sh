...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/mooyun/ssogga/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        last_version=$1
        url="https://github.com/mooyun/ssogga/releases/download/${last_version}/soga-cracked-linux64.tar.gz"
